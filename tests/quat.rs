...
macro_rules! impl_quat_tests {
    ($new:ident, $quat:ident, $vec3:ident, $vec4:ident, $t:ident) => {
        #[test]
        fn test_new() {
            let ytheta = deg(45.0);
...
mod quat {
    ...
    impl_quat_tests!(quat, Quat, Vec3, Vec4, f32);
}

mod dquat {
    use crate::support::deg;
    use glam::{dquat, DQuat, DVec4};
    ...
    impl_quat_tests!(dquat, DQuat, DVec3, DVec4, f64);
}
